...
```bash
san-lorenzo-backend$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
san-lorenzo-backend$ sam local start-api
san-lorenzo-backend$ curl http://localhost:3000/
```
...
```bash
san-lorenzo-backend$ sam build
san-lorenzo-backend$ sam deploy
```
...
```bash
san-lorenzo-backend$ sam logs -n NetCodeWebAPIServerless --stack-name san-lorenzo-backend --tail
```
...
```bash
san-lorenzo-backend$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name san-lorenzo-backend
```
