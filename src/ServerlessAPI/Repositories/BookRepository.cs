...
public class BookRepository(IDynamoDBContext context, ILogger<BookRepository> logger) : IBookRepository
{
    ...
    public async Task<bool> DeleteAsync(Book book)
    {
        ...
        try
        {
            ...
            var deletedBook = await context.LoadAsync<Book>(book.Id, new DynamoDBContextConfig
            {
                ConsistentRead = true
            });
